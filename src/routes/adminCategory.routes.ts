...
router.put('/edit', async (req: CustomRequestIo, res: Response) => {
  try {
    ...
    if (name === nameOld) {
      ...
      const fileDeletePath = `${config.get('fileCategoryPath')}\\${name}\\${oldNameFile!.name}`
      ...
      const oldFileDeletePath = `${config.get('fileCategoryPath')}\\${nameOld}\\${nameDeleteFile!.name}`
      fs.unlinkSync(oldFileDeletePath)
      // delete old dir
      fs.rmdirSync(`${config.get('fileCategoryPath')}\\${nameOld}`)
      console.log('---------ok1-------')
      const category = await Category.findByIdAndUpdate(id, 
        { name,         
          link: file.name, 
...
router.delete('/delete/:id/:adminId', async (req: CustomRequestIo, res: Response) => {      
  try {
    ...
    if (nameFileCategory) {
      const filePath = `${config.get('fileCategoryPath')}\\${nameFileCategory.name}\\${nameFileCategory.link}` 
      if (!fs.existsSync(filePath)) {
        res.status(400).json({message: 'Файла по пути нет'})
      } else {
        fs.unlinkSync(filePath)
        fs.rmdirSync(`${config.get('fileCategoryPath')}\\${nameFileCategory.name}`)
